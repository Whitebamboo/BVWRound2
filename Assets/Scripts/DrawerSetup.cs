...
public class DrawerSetup : MonoBehaviour
{
    ...
    private void StoreGrabInfo(XRBaseInteractor args)
    {
        ...
        grabPosition = args.attachTransform.transform.position;
    }
    ...
    private float FindPercentageDifference()
    {
        ...
        if(handle.hoveringInteractors.Count == 0)
        {
            Debug.LogError("Wrong");
            return 0;
        }

        Vector3 handPosition = handle.hoveringInteractors[0].transform.position;
